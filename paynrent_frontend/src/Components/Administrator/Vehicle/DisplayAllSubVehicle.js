...
export default function DisplayAllSubVehicle(){
  ...
  const [ratings,setRatings]=useState('')
   ...
   const fetchAllSubVehicle=async()=>{
  ...
  setFuelTypeId(rowData.fueltype)
  setAverage(rowData.average)
  setCapacityId(rowData.capacity)
 ...
 function DisplayAllSubVehicle() {
    return (
      <MaterialTable
        ...
        columns={[
          ...
          { title: 'Picture', field: 'icon', render: (rowData) => <Avatar src={`${ServerURL}/images/${rowData.icon}`} style={{ width: 55, height: 55 }} variant="rounded"></Avatar>  },
        ]}
        data={vehicle}        
        actions={[
          {
          ...
          },
          {
            ...
            tooltip: 'Add Vehicle',
            isFreeAction: true,
            onClick: () => navigate('/dashboard/subvehicle')
          }
         ]}
      />
    )
    ...
    var response=await getData('subvehicle/display_all_fueltype')
    ...
    var response=await getData('subvehicle/display_all_capacity')
    ...
    var response=await postData('subvehicle/delete_data',body,false)
    if(response.status)
   {
    Swal.fire({
        icon: 'success',
  ...
  // Update Button (Edit Data) //
    ...
    var response = await postData('subvehicle/edit_data', body)
    if (response.status) {
      Swal.fire({
        icon: 'success',
   ...
   // Save Picture  //
    ...
    var response = await postData('subvehicle/edit_picture', formData,true)
    if(response.result)
   {
   Swal.fire({
   icon: 'success',
           ...
           <DialogContent>
              <div className={classes.dialogbox}>
                 <Grid container spacing={2}>
                    ...
                    <Grid item xs={6} >
                        <TextField value={discount} fullWidth onChange={(event) => setDiscount(event.target.value)} label="Discount"></TextField>
                    </Grid>
